...
const ValidationErrorModal: React.FC<ValidationErrorModalProps> = ({ isOpen, onClose, errors }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={onClose}>
            <div className="bg-light-card dark:bg-dark-card rounded-xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-3 overflow-y-auto">
                    <p className="text-sm">Please correct the following errors before generating the ticket:</p>
                    <ul className="list-disc list-inside space-y-2 text-red-600 dark:text-red-400 bg-red-500/10 p-3 rounded-lg">
                        {errors.map((error, index) => (
                            <li key={index}>{error}</li>
                        ))}
                    </ul>
                </div>
                <div className="p-4 mt-auto border-t border-gray-200 dark:border-gray-700 flex justify-end">
