...
const PlaysTable: React.FC<PlaysTableProps> = ({ plays, updatePlay, deletePlay, selectedPlayIds, setSelectedPlayIds, onCopyWagers }) => {
  ...
  const renderInput = (play: Play, field: 'straightAmount' | 'boxAmount' | 'comboAmount') => {
    ...
    return (
        <input
            ...
            className="w-full bg-light-surface dark:bg-dark-surface p-1.5 rounded-md border-2 border-transparent focus:border-neon-cyan focus:outline-none text-center text-gray-900 dark:text-gray-200"
            placeholder="$"
            min="0"
          ...
          <tbody>
            {plays.length === 0 ? (
              ...
              plays.map((play, index) => {
                ...
                return (
                <tr key={play.id} className={`border-b border-gray-200 dark:border-gray-700 last:border-b-0 hover:bg-light-surface/50 dark:hover:bg-dark-surface/50 transition-colors ${isInvalid ? 'bg-red-500/10' : ''}`}>
                  <td className="p-2">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-2 text-center">
                    <button onClick={() => deletePlay(play.id)} className="text-red-500 hover:text-red-400">
                      <svg data-lucide="x-circle" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>
                    </button>
                  </td>
                </tr>
              )})
